...
scenarios('../features/SNMP.feature')
# scenarios('D:\pythonProject\adderCode1\tests\testcase10\features\SNMP.feature')
...
def user_enable_the_SNMP():
    driver.find_element_by_xpath('//*[@id="snmp_enabled_1"]').click()
    print("click on SNMP enable done")
    driver.execute_script("window.scrollTo(0,100)")
    time.sleep(2)

@then(parsers.parse('user enter NMS address "{NMS}"'))
def user_enter_NMS_address(NMS):
    driver.find_element_by_xpath('//*[@id="snmp_nms_address"]').clear()
    driver.find_element_by_xpath('//*[@id="snmp_nms_address"]').send_keys(NMS)

@then(parsers.parse('user enter the username "{name}"'))
def user_enter_the_username(name):
    driver.find_element_by_xpath('//*[@id="snmp_username"]').clear()
    driver.find_element_by_xpath('//*[@id="snmp_username"]').send_keys(name)

@then("user choose allowed connection method no authentication")
def user_choose_allowed_connection_method_no_authentication():
    driver.find_element_by_xpath('//*[@id="snmp_connection_method_noauthnopriv"]').click()
...
def user_enable_the_SNMP():
    driver.find_element_by_xpath('//*[@id="snmp_enabled_0"]').click()
    print("click on SNMP disable done")
    time.sleep(2)